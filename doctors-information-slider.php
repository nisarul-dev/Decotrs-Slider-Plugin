 ...
 */
function dis_theme_name_scripts() {
...
add_action( 'wp_enqueue_scripts', 'dis_theme_name_scripts' );
 ...
 */
function dis_doctors_slider_shortcode( $atts = [], $content = null, $tag = '' ) {
	$prefix = '_dis_';
 ...
 */
function dis_shortcodes_init() {
	if ( ! shortcode_exists( 'year' ) ) {
		add_shortcode( 'doctors-slider', 'dis_doctors_slider_shortcode' );
	}
}

add_action( 'init', 'dis_shortcodes_init' );

/**
 * Custom Post Type: Doctors Slider
 */
function dis_doctors_slider_post_type() {
	$labels = array(
...
add_action( 'init', 'dis_doctors_slider_post_type' );

/**
 * Custom Taxonomy: Specializations (Doctors Slider)
 */
function dis_specializations_taxonomy() {
	$labels = array(
...
add_action( 'init', 'dis_specializations_taxonomy' );


/**
 * Single Post Edit page title placeholder, "Add title" to "Doctor's Name"
 */
function dis_title_place_holder( $title, $post ) {
...
add_filter( 'enter_title_here', 'dis_title_place_holder', 20, 2 );

/**
 * CPT admin edit page, order by 'title' ASC.
 */
function dis_custom_post_type_admin_sort( $query ) {
	global $pagenow;
...
add_action( 'pre_get_posts', 'dis_custom_post_type_admin_sort' );